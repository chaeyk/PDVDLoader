                     ...
                     _In_ int       nCmdShow)
{
	...
	if (CreateProcessW(L"C:\\Program Files (x86)\\Steam\\steamapps\\common\\PowerDVD Ultra\\PowerDVD15\\PowerDVD.exe", NULL, NULL, NULL, TRUE, 0, NULL, NULL, &info, &processInfo))
	{
...
void ResizeMovieWindow(HWND hWnd, RECT* parentRect)
{
	...
	if (GetWindowPlacement(hWnd, &placement))
	{
		...
		LONG top = -heightDifference;
		LONG bottom = (LONG)(width * ratioWithBorders - heightDifference);
			...
			placement.rcNormalPosition.bottom != bottom)
		{
			...
			GetWindowPlacement(hWnd, &placement);
			TCHAR buf[128];
			_sntprintf_s(buf, _TRUNCATE, _T("%d %d %d %d\r\n"), placement.rcNormalPosition.left, placement.rcNormalPosition.right, placement.rcNormalPosition.top, placement.rcNormalPosition.bottom);
